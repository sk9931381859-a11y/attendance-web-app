...
export default function AdminSidebar({
  ...
  const renderContent = (isLight: boolean) => (
    <div className="flex flex-col h-full justify-between select-none">
            ...
            <div
              className={`w-9 h-9 rounded-xl flex items-center justify-center shadow-xs ${
                isLight
                  ...
                  : 'bg-emerald-600/20 border border-emerald-500/30 text-emerald-400 shadow-inner'
              }`}
            >
              <Building2 size={18} />
            </div>
          ...
          {navItems.map((item) => {
            ...
            if (isLight) {
              activeClass = item.isActive
                ...
                : 'text-slate-700 hover:bg-slate-100 hover:text-slate-900 font-medium transition-colors';
            } else {
              activeClass = item.isActive
            ...
            return (
              <Link
                ...
                href={item.href}
                onClick={onClose}
                className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm transition-all duration-150 group ${activeClass}`}
              >
                <Icon
                  size={18}
                  className={
      ...
      >
        ...
        >
          ...
          <div
            className={`text-sm font-mono font-bold tracking-wider flex items-center justify-between ${
              isLight ? 'text-slate-900' : 'text-white'
            }`}
          >
        ...
        {/* Quick Kiosk Launch link on mobile drawer */}
        {isLight && (
          <a
            href="/kiosk"
            target="_blank"
            ...
            onClick={onClose}
            className="flex items-center justify-between px-3 py-2 rounded-lg bg-teal-50 hover:bg-teal-100 text-xs font-semibold text-teal-800 border border-teal-200 transition"
          >
            <span className="flex items-center gap-2">
              <QrCode size={14} className="text-teal-600" />
              <span>Launch Front Desk Kiosk</span>
            </span>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 md:hidden flex animate-in fade-in duration-200">
          <div
            ...
            onClick={onClose}
          />
          <aside className="relative w-72 max-w-[85vw] bg-white h-full shadow-2xl z-10 flex flex-col border-r border-slate-200">
            {renderContent(true)}
          </aside>
        </div>
      )}
